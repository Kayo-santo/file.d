...
func newMetricCollection(
	possibleOffsetCorruptionMetric, errorOpenFileMetric, invalidStreamLogsMetric prometheus.Counter,
...
func NewJobProvider(config *Config, metrics *metricCollection, sugLogger *zap.SugaredLogger) *jobProvider {
	jp := &jobProvider{
		config:   config,
		offsetDB: newOffsetDB(config.OffsetsFile, config.OffsetsFileTmp, newOffsetDbMetricCollection(metrics.invalidStreamLogsMetric)),
